...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="carscars" && $field=="Picture" && $ptype=="list")
	{
		$arr = my_json_decode($value);
$value = "<div class='list_image' style='background-image:url(".GetWebRootPath().$arr[0]["name"].")'></div>";
;
		return $value;
	}
				if($table=="carscars" && $field=="Price")
	{
		$value = "<span class='dollarsym'>$</span>".number_format(intval($value), 0, '.', ',');;
		return $value;
	}
	return $value;
}
